...
export function ouvrirInventaire(demande: DemandeInventaire, utilisateurId: number): Inventaire {
  ...
  const enCours = base().prepare("SELECT reference FROM inventaires WHERE statut = 'en_cours'").get() as
    | { reference: string }
    | undefined
  if (enCours) {
  ...
  return transaction(() => {
    ...
    const params: Record<string, unknown> = { inv: inventaireId }
    ...
    const nb = (
      base().prepare('SELECT COUNT(*) n FROM inventaire_lignes WHERE inventaire_id = ?').get(inventaireId) as {
        n: number
      }
    ).n
...
export function saisirComptage(
  ...
  const ligne = base()
    ...
    .get(ligneId) as (InventaireLigne & { statut: string; inventaire_id: number }) | undefined
...
export function validerInventaire(inventaireId: number, utilisateurId: number): ResultatValidation {
  const inv = base().prepare('SELECT * FROM inventaires WHERE id = ?').get(inventaireId) as Inventaire | undefined
...
export function inventaire(id: number): (Inventaire & { lignes: InventaireLigne[] }) | null {
  const inv = base()
    ...
    .get(id) as Inventaire | undefined
  if (!inv) return null
  ...
  const lignes = base()
    ...
    .all(id) as InventaireLigne[]

  return { ...inv, lignes }
}

export function inventaireEnCours(): Inventaire | null {
  const l = base().prepare("SELECT id FROM inventaires WHERE statut = 'en_cours'").get() as
    | { id: number }
    | undefined
